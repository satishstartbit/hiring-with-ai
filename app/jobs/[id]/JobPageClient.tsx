...
export default function JobPageClient({ job }: Readonly<{ job: PublicJob }>) {
  ...
  const handleSearchChange = useCallback((term: string) => {
    setSearchTerm(term);
    setCandidatesData((prev) => ({ ...prev, page: 1 }));
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!res.ok) return;
        const result = (await res.json()) as CandidatesResponse;
        if (!cancelled) setCandidatesData(result);
      } catch (error) {
  ...
  }, [job._id, candidatesData.page, searchTerm]);
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex shrink-0 items-center gap-3">
              <span
                className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-bold capitalize ${statusStyle.badge}`}
              >
          ...
          <aside className="space-y-4">
            <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className="mt-4 space-y-3 text-sm text-slate-700">
                ...
                <div className="flex justify-between text-slate-500">
                  <span>Status</span>
                  <span className="font-semibold capitalize text-slate-900">{job.status}</span>
                </div>
              </div>
            </div>
          </aside>
