...
export default function JobCard({ job }: Readonly<{ job: Job }>) {
  ...
  const acceptsApplicants = job.status === "active" || job.status === "ai_generated";
        ...
        <div className="mt-auto pt-5">
          <div className="mb-4 flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-2xl font-bold text-red-700">{job.applicantCount}</p>
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {acceptsApplicants && (
              <Link
                ...
                className="flex-1 rounded-md bg-red-600 px-3 py-2 text-center text-sm font-bold text-white transition-colors hover:bg-red-700"
              >
                Apply
              </Link>
            )}
...
function MetaPill({ label, value }: Readonly<{ label: string; value: string }>) {
  return (
